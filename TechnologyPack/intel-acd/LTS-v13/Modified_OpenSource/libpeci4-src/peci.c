                                 ...
                                 uint8_t u8WriteLen, int peci_fd, uint8_t* cc)
{
    ...
    memcpy(&cmd.value[0], (char *)&u32Value, sizeof(u32Value));
#else
    cmd.value = u32Value;
#endif
...
/*-------------------------------------------------------------------------
 * This function allows sequential peci_WrEndPointConfig to PCI MMIO with
 * the provided peci file descriptor.
 *------------------------------------------------------------------------*/
EPECIStatus peci_WrEndPointConfigMmio_seq(uint8_t target, uint8_t u8MsgType,
                                      uint8_t u8Seg, uint8_t u8Bus,
                                      uint8_t u8Device, uint8_t u8Fcn,
                                      uint8_t u8Bar, uint8_t u8AddrType,
                                      uint64_t u64Offset, uint8_t DataLen,
                                      uint64_t DataVal, int peci_fd,
                                      uint8_t* cc)
{
    struct peci_wr_end_pt_cfg_msg cmd;
    EPECIStatus ret;
    ...
    // Per the PECI spec, the write length must be a byte, word, or dword or qword
    if (DataLen != 1 && DataLen != 2 && DataLen != 4 && DataLen != 8)
    {
        return PECI_CC_INVALID_REQ;
    }

    // Per the PECI spec, the address type must be either 0x5 or 0x6
    if (u8AddrType != PECI_ENDPTCFG_ADDR_TYPE_MMIO_D && u8AddrType != PECI_ENDPTCFG_ADDR_TYPE_MMIO_Q)
    {
        return PECI_CC_INVALID_REQ;
    }
...
/*-------------------------------------------------------------------------
 *  This function provides write access to the EP PCI MMIO
 *------------------------------------------------------------------------*/
EPECIStatus peci_WrEndPointConfigMmio(uint8_t target, uint8_t u8Seg,
                                     uint8_t u8Bus, uint8_t u8Device,
                                     uint8_t u8Fcn, uint8_t u8Bar,
                                     uint8_t u8addrType, uint64_t u64Offset,
                                     uint8_t DataLen, uint64_t DataVal,
                                     uint8_t* cc)
{
    int peci_fd = -1;
    EPECIStatus ret;
    ...
    ret = peci_WrEndPointConfigMmio_seq(target, PECI_ENDPTCFG_TYPE_MMIO, u8Seg,
                                    u8Bus, u8Device, u8Fcn, u8Bar, u8addrType, u64Offset,
                                    DataLen, DataVal, peci_fd, cc);
    peci_Close(peci_fd);
    return ret;
}
...
{
    int peci_fd = -1;
    struct peci_telemetry_disc_msg cmd = {};
    EPECIStatus ret = PECI_CC_SUCCESS;

    if (pData == NULL || cc == NULL)
    {
        printf("peci_Telemetry_Discovery 1 \n");//test line
        return PECI_CC_INVALID_REQ;
    }
    ...
    {
         printf("peci_Telemetry_Discovery 2 \n");//test line
        return PECI_CC_INVALID_REQ;
    }
    ...
    if (sizeof(cmd.data) < u8ReadLen)
    {
        printf("peci_Telemetry_Discovery 3 \n");//test line
        return PECI_CC_INVALID_REQ;
    }

    if (peci_Open(&peci_fd) != PECI_CC_SUCCESS)
    {
        printf("peci_Telemetry_Discovery 4 \n");//test line
        return PECI_CC_DRIVER_ERR;
    }
...
{
    int peci_fd = -1;
    struct peci_telemetry_get_telem_sample_msg cmd = {};
    EPECIStatus ret = PECI_CC_SUCCESS;
...
EPECIStatus peci_Telemetry_ConfigWatcherRd(uint8_t target, uint16_t watcher,
                                           ...
                                           uint64_t* pData, uint8_t* cc)
{
    int peci_fd;
    struct peci_telemetry_config_watcher_msg cmd = {};
    EPECIStatus ret;
...
{
    int peci_fd = -1;
    struct peci_telemetry_get_crashlog_sample_msg cmd = {};
    EPECIStatus ret = PECI_CC_SUCCESS;
