...
static acdStatus GetCPUID(CmdInOut* cmdInOut)
{
    ...
    cmdInOut->out.val.u64 = ((uint64_t)cmdInOut->out.cpuID.cpuModel) |
                            (uint64_t)cmdInOut->out.cpuID.stepping;
    UpdateInternalVar(cmdInOut);
    return ACD_SUCCESS;
}
...
static acdStatus RdEndpointConfigPci(CmdInOut* cmdInOut)
{
    ...
    if (!IsRdEndpointConfigPCIParamsValid(cmdInOut->in.params,
                                          cmdInOut->validatorParams))
    {
        return ACD_INVALID_RDENDPOINTCONFIGPCI_PARAMS;
    }
    ...
    switch (params.rx_len)
    {
        ...
        case sizeof(uint16_t):
        case sizeof(uint32_t):
            cmdInOut->out.ret = peci_RdEndPointConfigPci(
        ...
        ACD_SUCCESS)
    {
        if (getIsTelemetrySupportedResult(cmdInOut))
        {
            return logCrashlogSection(
                ...
                getNumberofCrashlogAgents(cmdInOut), *cmdInOut->runTimeInfo);
        }
    }
    else
    {
...
static acdStatus RdGlobalVars(CmdInOut* cmdInOut)
{
    ...
    if (mismatchcrashcorecount == 0)
    {
        ...
        cmdInOut->out.val.u64 = (uint64_t)(__builtin_popcountll(
				cmdInOut->cpuInfo->crashedCoreMask));
        return ACD_SUCCESS;
    }
    //------AMI start-------
