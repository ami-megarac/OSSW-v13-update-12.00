...
int m_btn_status;
int seqno;
static bool caps_lock_persistent = false;
...
void CloseUSBDevice(void) {
	if (gUsbDevice > 0) {
		...
		close(gUsbDevice);
		if( safe_system(REMOVE_IUSB_KEY_FILE) < 0 ) {
...
int mousePkt(int buttonMask, int x, int y, int maxx, int maxy) {
	...
	memcpy((char*) pkt->Signature, IUSB_SIG, sizeof(pkt->Signature));
	pkt->Major = 0x01;
	pkt->Minor = 0;
	pkt->HeaderLen = 32;
	...
	short X = (short) ((double)(x * DIRABS_MAX_SCALED_X) / maxx/*width*/+ 0.5);
	short Y = (short) ((double)(y * DIRABS_MAX_SCALED_Y) / maxy/*height*/+ 0.5);
	...
	mouseRep.Y = Y;
	mouseRep.Scroll = 0;
    /* Reason for false positive -The number of bytes attempt to be copied will be limited to MAX_MOUSE_REPORT_SIZE*/
...
#else
	/* Reason for false positive - The function is passing a 6-byte data into 'sendmousedata' by typecasting it to u8*.
	/* coverity[overrun-buffer-arg : FALSE] */
...
void handleKbdEvent(int8 down, uint32 key) {
	int shiftPressed = (prevkey.Modifier & MOD_LEFT_SHIFT) || (prevkey.Modifier & MOD_RIGHT_SHIFT);
	//To Handle CAPS letter
	if (key >= 'A' && key <= 'Z' && down == 1) {
		if (shiftPressed == 0 && !caps_lock_persistent) {
			if (Tightvnctextmode) {
				caps_lock_persistent = true;
				SendKeyEvent(CAPS_LOCK_KEY_CODE, 1);
				SendKeyEvent(CAPS_LOCK_KEY_CODE, 0);
			}
		}
	} else if (key >= 'a' && key <= 'z' && down == 1) {
	// To Handle Small letter
		if (shiftPressed == 0) {
			if (Tightvnctextmode && caps_lock_persistent){
				caps_lock_persistent = false;
				SendKeyEvent(CAPS_LOCK_KEY_CODE, 1);
				SendKeyEvent(CAPS_LOCK_KEY_CODE, 0);
			}
		}
...
void SendKeyEvent(uint32 key, int8 down){
    ...
    /* Reason for false positive -"KeybdInfo" is defined within the USB_KEYBD_PKT structure, but it is being used to store values from the IUSB_HID_PACKET structure*/
    /* coverity[overrun-buffer-val : FALSE] */
...
#else
    if((ret = SendKeybdData((UINT8*)&KeybdInfo[0])) < 0) {
