...
#if !defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
...
uint32_t total_interrupt_sensors = 0;
...
uint32_t total_chassis_interrupts = 0;
...
{
    pending_interrupt_info *newnode = NULL;      /* New node to be inserted */
...
static inline void remove_interrupt(char* type, pending_interrupt_info **front, pending_interrupt_info **rear)
{
    ...
    /*Reason for False Positive - If the pointer of "front" or "rear" is not null, 
                                  it means the Queue is not empty.  */
...
process_gpio_intr (int gpio_num, int gpio_port_pin_offset, unsigned char state, unsigned char trigger_type)
{
	...
	for(j = 0; j < total_interrupt_sensors; j++)
	{
		...
		{
			intr_sensors[j].reading_on_assertion = state;
	...
	process_gpio_intr_ext(gpio_num+gpio_port_pin_offset,state,trigger_type);
#if 0 // remved needless dbgprint
	else
...
{	
	...
	wakeup_intr_queue_ext(intr_type);
}

int
gpio_verify (int gpio_num)
{   
    ...
    for(j = 0; j < total_interrupt_sensors; j++)
    {
        if(gpio_num == intr_sensors[j].gpio_number)
            return 1;
    }
    for(j = 0; j < total_chassis_interrupts; j++)
    {
        if(gpio_num == intr_chassis[j].gpio_number)
            return 1;
    }

    return gpio_verify_ext(gpio_num);
...
gpio_ioctlUnlocked(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    ret = gpio_ioctlUnlocked_ext(pdev,cmd,arg);
    if(ret != -EINVAL)
    {
        return ret;
    }
	...
	case REGISTER_SENSOR_INTS:
    {
        ...
        for(i=0;i<total_interrupt_sensors;i++)
        {
            /*Reason for False Positive - Compare member of unsigned short, ensure value is 0 ~ 255 */
	...
	case UNREGISTER_SENSOR_INTS:
    {
        ...
        for(i=0;i<total_interrupt_sensors;i++)
        {
            gpin_data.PinNum = intr_sensors[i].gpio_number;
	...
	case UNREGISTER_CHASSIS_INTS:
	   {
	        ...
	        for(i=0;i<total_chassis_interrupts;i++)
	        {
	            gpin_data.PinNum = intr_chassis[i].gpio_number;
	            if(g_gpiopinlist[gpin_data.PinNum] == 1)
	            {
	...
	case READ_GPIOS:
		if (pdev->pgpio_hal->pgpio_hal_ops->readgpios) {
			...
			if (gpin_data.gpio_list.count == 0 || gpin_data.gpio_list.count > GPIO_MAX_COUNT)
				return -EINVAL;

			if (check_mul_overflow((size_t)gpin_data.gpio_list.count, sizeof(gpio_list_data_info), &bytes))
				return -EOVERFLOW;
	...
	case SET_GPIO_PROPERTY:
        if (pdev->pgpio_hal->pgpio_hal_ops->set_gpio_property)
        {
            if (0 != pdev->pgpio_hal->pgpio_hal_ops->set_gpio_property(gpin_data.PinNum, gpin_data.property.id, gpin_data.data))
            {
                TWARN("GPIO set property failed, PinNum: %i, ID: %X, data: %X \n", gpin_data.PinNum, gpin_data.property.id, gpin_data.data);
                return -1;
            }
        }
		break;
	case GET_GPIO_PROPERTY:
        if (pdev->pgpio_hal->pgpio_hal_ops->get_gpio_property)
        {
            if(0 != pdev->pgpio_hal->pgpio_hal_ops->get_gpio_property(gpin_data.PinNum, gpin_data.property.id, &gpin_data.data))
            {
                TWARN("GPIO get property failed, PinNum: %i, ID: %X, data: %X \n", gpin_data.PinNum, gpin_data.property.id, gpin_data.data);
                return -1;
            }
            if(copy_to_user((void __user *)arg, (void*) &gpin_data, sizeof(Gpio_data_t)))
                return -EFAULT;
...
// New ioctl calling the pulse interface
	case PULSE_GPIO:
		if (pdev->pgpio_hal->pgpio_hal_ops->writegpio_pulsedata) 
		{
			if (0 != pdev->pgpio_hal->pgpio_hal_ops->writegpio_pulsedata((void*)&gpin_data ))
				return -1;
		}
		break;

	default :
		TWARN ( "Invalid GPIO Function %x\n",cmd);
		return -EINVAL;
	}
       return 0;
