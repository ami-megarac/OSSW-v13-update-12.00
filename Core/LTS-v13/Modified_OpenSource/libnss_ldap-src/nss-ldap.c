			...
			char *buffer, size_t buflen, int *errnop)
{
	...
	if (nss_res.action == PAM_RESPONSE)
	{
		...
		{
			if(0 != IsLDAPUser(nss_res.usr.name) )
                        {
	...
	{
		ADD_STRTOBUF_REF(pwd->pw_shell, defshell, init_buf, buffer, buflen);
	}
    ADD_STRTOBUF_REF(pwd->pw_dir, USER_DIR, init_buf, buffer, buflen);
			...
			char *buffer, size_t buflen, int *errnop)
{
	...
	if (nss_res.action == PAM_RESPONSE)
	{
		if (nss_res.ret == PAM_USER_RETRIEVED_SUCCESSFULLY)
		{
#ifdef CONFIG_SPX_FEATURE_SOLSSH_SUPPORT
            if (access("/tmp/NotifyUARTMirrorLogFlag", F_OK) != 0)
#endif
            {
			    if(0 != IsLDAPUser(nss_res.usr.name) )
                        {
