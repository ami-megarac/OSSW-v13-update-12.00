    ...
    while ( (def_ini && (fgets(lin, ASCIILINESZ, def_ini) != NULL)) || (ini && (fgets(lin, ASCIILINESZ, ini) != NULL)) ) { /* Fortify [Buffer overflow]:: False Positive */
        ...
        if (*where==';' || *where=='#' || *where==0)
		    ...
		    if(lwc_count < 0 || lwc_count >= (int)sizeof(sec)) {
			    TDBG("Buffer Overflow in File:%s Line: %d \n", \
				   __FILE__, __LINE__);
			    if(handler != NULL) {
				    if(!handler->hashTable)
...
void iniparser_add_entry(INIHandler *handler, char *sec, char *key, char * val)
{
    ...
    if (key!=NULL) {
        ...
        {
            TDBG("Buffer Overflow in File:%s Line: %d  Function : %s\n",__FILE__, __LINE__, FUNCTION_NAME);
            return;
        }
    } else {
...
void iniparser_setstring(INIHandler *handler, char *hashstr, char * val)
{
	...
	/*for eg: set username or Domain name in RIS configuration command in which max bytes is 256*/
	    ...
	    {
		    TDBG("Buffer Overflow in iniparser_setstring function\n");
		    return;
	    }
        }
...
void iniparser_dump_ini(INIHandler *handler, FILE *file)
{
    ...
    for (i=0 ; i < nsec ; i++) {
        ...
        {
            TDBG("Buffer Overflow in File:%s Line: %d  Function : %s for i=%d\n",__FILE__, __LINE__, FUNCTION_NAME,i);
            continue;
        }
        for (j=0 ; j < HASH_TABLE_SIZE ; j++) {
            if (hashTable[j] == NULL)
...
int Checkfile_ext (const char* filename, const char* ext)
{
	char* str = NULL;
	str = strrchr(filename,'.');
	if(str != NULL)
	{
		if(strncmp(str, ext, strlen(ext)) == 0)
		{
			return TRUE;
		}
	}
...
int AllowOtherUserToRead(char *filename)
{
    char AllowOtherUserToReadFileList[FILENAME_STRMAX] = "/conf/ncml.conf";

    if(strcmp(filename, AllowOtherUserToReadFileList) == 0)
    {
	#if !defined (_WIN32) && !defined (WIN64)    
	if(chmod(filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) != 0)
	{
		TDBG("[ERROR] IniFile: unable to change the permission for %s\n",filename);
		return -1;
	}
	#endif
        return 0;
...
int iniparser_dump_file(INIHandler *handle, char *filename)
{
    ...
    if ( -1 == rename(bkp_filename,filename) )
        TDBG("[ERROR] IniSaveFile: Name changed %s to %s Failed\n", bkp_filename, filename);
    
    if ((Checkfile_ext(filename,".ini") == TRUE) || (Checkfile_ext(filename,".conf") == TRUE))
    {
        /* There is no need to change '/conf/ncml.conf' file permission to 600, because when using solssh, the logged-in IPMI user must read this conf file.  */
        if(AllowOtherUserToRead(filename) == 0)
        {
            return 0;
        }
	#if !defined (_WIN32) && !defined (WIN64)    
        else if(chmod(filename, S_IRUSR | S_IWUSR) != 0)
    	{
    		TDBG("[ERROR] IniSaveFile: unable to change the permission for %s\n",filename);
    	}
	#endif
    }
...
int main (int argc, char *argv[])
{
    ...
    if (argc != 2) {
        TINFO ("Usage:%s <test ini file name> \n", argv[0]);
        return 0;
    }

    INIHandler *iniHandler;
    TDBG ("Loading the ini\n");
    iniHandler = iniparser_load(argv[1]);

    if (iniHandler == NULL) {
        TDBG ("Unable to load ini : %s\n", argv[1]);
        return 0;
    }
