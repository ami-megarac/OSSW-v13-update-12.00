...
IniLoadFile(char *filename)
{
    ...
    else
    {
        ...
        if (fp == NULL)
        {
            TDBG("[ERROR] LoadIniFile: %s creation Failed\n",filename);
            return NULL;
        }
        fclose(fp);
        if (Checkfile_ext(filename,".ini") == TRUE)
        {
        	if(chmod(filename, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH) != 0)
        	{
        		TDBG("[ERROR] LoadIniFile: unable to change the permission for %s\n",filename);
        	}
        }
    }
    ...
    if (d == NULL)
    {
        TDBG("[ERROR] LoadIniFile: %s open  Failed\n",filename);
        return NULL;
    }
...
IniSaveFile(INI_HANDLE handle,char *filename)
{
	...
	/* Save the INI in a backup file. So during failure (poweroff/reset) 
	   during write will not corrupt original file */
    	...
    	if(fp == NULL)
    	{
		TDBG("[ERROR] SaveIniFile: %s open  Failed\n",filename);
        	return -1;
    	}
