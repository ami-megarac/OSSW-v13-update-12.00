...
{
	int len;
	char *buffer = (char*)kcalloc(1,2080,GFP_KERNEL);
...
#endif
{
	...
	*eof = 1;
	...
	for (i = 0; i < 0x98; i += 4) {
		len += sprintf(buffer + len, "%03X: %08X\n", i, ast_videocap_read_reg(i));
	}
...
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
	ast_videocap_sysctl = register_sysctl(AST_VIDECAP_PRCO_DIR, ast_videocap_sysctl_table);
#else
...
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
	unregister_sysctl_table(ast_videocap_sysctl);
#else
