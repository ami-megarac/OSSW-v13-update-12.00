...
static struct class *ast_videocap_class;
static struct device *ast_videocap_device;
#else
...
static int ast_videocap_alloc_mem_pool(void)
{
	...
	unsigned long size;
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
	ast_videocap_video_buf_virt_addr = dma_alloc_coherent(ast_videocap_device, MEM_POOL_SIZE, &dma, GFP_KERNEL);
#else
	...
	while (size > 0) {
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
		SetPageReserved(virt_to_page((const void *)addr));
#else
...
static void ast_videocap_free_mem_pool(void)
{
	...
	if (ast_videocap_video_buf_virt_addr != NULL) {
		...
		while (size > 0) {
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
			ClearPageReserved(virt_to_page((const void *)addr));
#else
...
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
		dma_free_coherent(ast_videocap_device, MEM_POOL_SIZE, ast_videocap_video_buf_virt_addr, dma);
#else
...
int __init ast_videocap_module_init(void)
{
	...
	printk("AST Video Capture Driver, (c) 2009-2025 American Megatrends Inc.\n");
	...
	if (ret < 0) {
		printk(KERN_ERR "%s: failed to create char device\n", AST_VIDEOCAP_DEV_NAME);
		goto out_cdev_register;
	}
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
	ast_videocap_class = class_create(AST_VIDEOCAP_DEV_NAME);
	if (IS_ERR(ast_videocap_class))
	{
		printk(KERN_ERR "%s: failed to create char class\n", AST_VIDEOCAP_DEV_NAME);
		goto out_cdev_add;
	}

	ast_videocap_device = device_create(ast_videocap_class, NULL, ast_videocap_devno, NULL, AST_VIDEOCAP_DEV_NAME);
	if (IS_ERR(ast_videocap_device))
	{
		printk(KERN_ERR "%s: failed to add char device\n", AST_VIDEOCAP_DEV_NAME);
		goto out_class_create;
	}

	ast_videocap_device->coherent_dma_mask = DMA_BIT_MASK(32);
	ast_videocap_device->dma_mask = &ast_videocap_device->coherent_dma_mask;
#else
	...
	/* Fixes clock handling logic w.r.t SDKv08.03 video driver */
	ret = clk_prepare_enable(vclk);
	if (ret != 0) {
		printk(KERN_ERR "Failed to Handle vclk clock\n");
		goto out_iomap;
	}
	ret = clk_prepare_enable(eclk);
	if (ret != 0) {
		printk(KERN_ERR "Failed to Handle eclk clock\n");
		goto out_iomap;
	}
#endif
...
#if defined(CONFIG_SPX_FEATURE_GLOBAL_KERNEL_VER6)
out_device_create:
	device_destroy(ast_videocap_class, ast_videocap_devno);
out_class_create:
	class_destroy(ast_videocap_class);
#else
