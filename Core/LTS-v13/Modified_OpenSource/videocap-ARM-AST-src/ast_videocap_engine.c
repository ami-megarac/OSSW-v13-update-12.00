...
int VideoCapture(struct ast_videocap_engine_info_t *info)
{
	...
	unsigned long count = ast_videocap_read_reg(AST_VIDEOCAP_COMPRESSED_DATA_COUNT);
	if (count > (AST_VIDEOCAP_COMPRESS_BUF_SZ / AST_VIDEOCAP_COMPRESSED_DATA_COUNT_UNIT)) {
		// Handle error - value too large
		info->CompressData.CompressSize = 0;
		return ASTCAP_IOCTL_ERROR;
	}
	info->CompressData.CompressSize = count * AST_VIDEOCAP_COMPRESSED_DATA_COUNT_UNIT;
