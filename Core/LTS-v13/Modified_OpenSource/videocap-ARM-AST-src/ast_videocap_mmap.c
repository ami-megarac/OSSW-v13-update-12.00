...
int ast_videocap_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	upper_bound = (unsigned long) (ast_videocap_video_buf_phys_addr - \
	(AST_VIDEOCAP_IN_BUF_SZ + AST_VIDEOCAP_IN_BUF_SZ + AST_VIDEOCAP_FLAG_BUF_ALLOC_SZ));

	if ((vma->vm_end - vma->vm_start)  > upper_bound) 
	{
		 printk("vm size exceeds boundry \n");
		 return -ENXIO;
	}
	
	/* Check if the offsets are aligned */
	if ((VMA_OFFSET(vma)) & (PAGE_SIZE)) {
	...
	vma->vm_flags |= VM_IO |  VM_WRITE | VM_READ;
#else
