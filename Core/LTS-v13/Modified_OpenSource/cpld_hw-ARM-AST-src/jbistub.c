...
void extern_delay(unsigned int nval)
{
	static unsigned int extern_idle_count = 0;
	//msleep :: for kernel switch other task.
	if((++ extern_idle_count ) % 262144 == 0)
	{ //per 256k will perform msleep(0).
		msleep(0);
		extern_idle_count = 0;
	}
	else
...
int jbi_jtag_io(int tms, int tdi, int read_tdo)
{
    ...
    Pin_ctrl(TCK_PIN, 1);
		...
		extern_delay(CONFIG_SPX_FEATURE_ALTERA_CPLD_EXTEND_CLK_HOLD_TIME);
    #endif
...
int jbcmain(char* jbc_action,unsigned long *buf, unsigned long data_size,unsigned char IsBackground)
{
	...
	JBI_RETURN_TYPE retry_exec_result = JBIC_SUCCESS;
				...
				*/
				...
				if (exec_result == JBIC_SUCCESS)
				{
                    ...
                    if((fg_Data_Readback == 0) && (exit_code != 0)) {
						if (exit_code == 11) {
							dbgprintf("Retry device verify");
							action = (char*)"VERIFY";
							retry_exec_result = jbi_execute(file_buffer, file_length, workspace,
							workspace_size, action,init_list, reset_jtag,
							&error_address, &exit_code, &format_version);
							
							if (retry_exec_result == JBIC_SUCCESS)
							{
								switch (exit_code)
								{
								case  0: exit_string = "Success"; break;
							...
							if (exit_code != 0) {
								dbgprintf("Verification retry failed\n");
