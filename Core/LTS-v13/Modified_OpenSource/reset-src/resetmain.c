...
extern wait_queue_head_t * tmp_reset_wait_queue;
extern unsigned int reset_event;
static unsigned int reset_wait_count = 0;
DEFINE_MUTEX(reset_mutex);
int process_reset_intr(void);
...
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,4,11))
        {.procname="DebugLevel",.data=&(TDBG_FORM_VAR_NAME(reset)),.maxlen=sizeof(int),.mode=0644,.proc_handler=&proc_dointvec},
#else
...
static char banner[] __initdata = KERN_INFO "Reset Common Driver, (c) 2009-2015 American Megatrends Inc.\n";
...
reset_ioctl_unlocked(struct file *file, unsigned int cmd, unsigned long arg)
{
       ...
       printk("Reset ioctl unlocked\n");
		...
		case WAIT_ON_RESET:
			...
			ret = wait_event_interruptible( * tmp_reset_wait_queue, reset_event != 0 );
			mutex_lock(&reset_mutex);
			reset_wait_count --;
			mutex_unlock(&reset_mutex);
...
{
	return reset_ioctl_unlocked(file,cmd,arg);
}
#endif
#endif
...
ResetCallBack(void)
{
	...
	for(i=0;i<RESET_MAX_CALLBACKS;i++)
	{
		if (NULL != m_reset_callback[i].CallBackHandler)
			(*(m_reset_callback[i].CallBackHandler)) ();
	}
	return 0;
}
...
#else
	my_sys = register_sysctl("reset",LPCRESET_table);
#endif
...
int reset_core_loaded =1;
EXPORT_SYMBOL(reset_core_loaded);
