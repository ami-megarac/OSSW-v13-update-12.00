...
/* This is a bit of a shameless hack, but, well, it works. */
    if (argc == 3)
    {
        ...
        if ((argv[2] != NULL) && (strlen(argv[2]) < 20))
        {
            ...
            {
                TCRIT("Error: Source size= %s is larger than Destination Size\n",argv[2]);
                return -1;
            }
        }
        else if (argv[2] == NULL)
	{	   
            openlog("timeoutd", OPENLOG_FLAGS, LOG_DAEMON);
            syslog(LOG_ERR, "Incorrect invocation of timeoutd, invalid device.");/*SCA Fix [Unbounded source buffer]:: False Positive */
		...
		syslog(LOG_NOTICE,
    		...
    		TDBG("Please try again tomorrow.\r\n");
    		*/
    		logoff_msg(1);
    		exit(10);
            case NOLOGIN:
...
void read_wtmp()
{
    ...
    {
        TCRIT("fseek: Unable to reachour EOF\n");
        fclose(fp);
        return;
    }
...
#ifdef DEBUG_WTMP
    struct tm	*tm;
    tm = localtime((const time_t *)&(wtmplist->elem.ut_time));
    TDBG("%d:%d %s %s %s\n", 
...
void read_config()
{
	...
	while (config[i])
	{
		...
		j = 0;
		  ...
		  TDBG("%d(%d-%d):", config[i]->times[j].days,
		  			config[i]->times[j].starttime,
		  			config[i]->times[j].endtime),j++;
		TDBG("%s:%s:%s:%s:%d;%s:%d;%s:%d;%s:%d\n",
			config[i]->ttys,
			config[i]->users,
			config[i]->groups,
...
char *in_set;
{
    ...
    if (t == NULL)
    {
        TDBG("timeoutd.c: No tokens found\n");
        free(set);
        return 0;
    }
    if(strlen(t) > strlen(in_set)) /* Avoid Buffer overflow */
    {
        TDBG("timeoutd.c:Line no-717: Buffer overflow\n");
        free(set);
        return 0;
    }
    while (t)
...
#ifdef DEBUG_WTMP
    struct tm	*tm;
    tm = localtime((const time_t *)&(login_p->elem.ut_time));
 ...
 */
    ...
    if (logout_p)
    {
      tm = localtime((const time_t *)&(logout_p->elem.ut_time));
      fprintf(stderr, "%d:%d %s %s %s\n", 
