...
void NAME##_func(regmatch_t MATCHES[]) { \
        ...
        if(sess == NULL){ \
            ...
            printf("Set-Cookie: QSESSIONID=; path=/; secure;HttpOnly;Samesite=Lax\n"); \
            qcgires_setcontenttype(req, "application/json"); \
            jresp = json_object_new_object(); \
        ...
        if(sess->getint(sess, "authorized")!=1 || getenvptr == NULL || (strlen(getenvptr) != strlen(sess->getstr(sess, "CSRFTOKEN", false)) ) || (strncmp(sess->getstr(sess, "CSRFTOKEN", false), getenvptr, strlen(sess->getstr(sess, "CSRFTOKEN", false))) != 0 ) ||/* check password change status for first login */ (sess->getint(sess, "passwordStatus")!=0) || (sess->getint(sess, "TFAEnabled")==1 && sess->getint(sess, "TFAStatus")==0)) { /*false positive [Memory Leak]*/\
                json_object *jresp; \
                printf("Status: 401 Unauthorized \n"); \
                printf("Set-Cookie: QSESSIONID=; path=/; secure;HttpOnly;Samesite=Lax\n"); \
                qcgires_setcontenttype(req, "application/json"); \
        ...
        pre_authorized_call_hook(req, sess);
...
#define START_HANDLER_AUTHORIZED_PASSWORD_RESET(NAME, METHOD, REGEX, NUM, MATCHES, HEADERS) \
void NAME##_func(); \
handler NAME##_data = {.func=NAME##_func,.method=METHOD,.regex_str=REGEX,.regex={.__buffer=0,.__allocated=0,.__used=0,.__syntax=0,.__fastmap=0,.__translate=0,.__can_be_null=0,.__regs_allocated=0,.__fastmap_accurate=0,.__no_sub=0,.__not_bol=0,.__not_eol=0,.__newline_anchor=0},.nmatch=NUM,.next=NULL};\
handler *NAME = &NAME##_data; \
void NAME##_func(regmatch_t MATCHES[]) { \
        ...
        if(sess->getint(sess, "authorized")!=1 || getenvptr == NULL || (strlen(getenvptr) != strlen(sess->getstr(sess, "CSRFTOKEN", false)) ) || (strncmp(sess->getstr(sess, "CSRFTOKEN", false), getenvptr, strlen(sess->getstr(sess, "CSRFTOKEN", false))) != 0 ) ||/* check password change status for first login */ (sess->getint(sess, "passwordStatus")!=1) ) { /*false positive [Memory Leak]*/\
                json_object *jresp; \
                printf("Status: 401 Unauthorized \n"); \
                printf("Set-Cookie: QSESSIONID=; path=/; secure;HttpOnly;Samesite=Lax\n"); \
                qcgires_setcontenttype(req, "application/json"); \
...
#define END_HANDLER_AUTHORIZED_PASSWORD_RESET \
end_handler_auth: \
		if(headers){}; \
        if(sess != NULL){ \
            post_authorized_call_hook(req, sess); \
...
void NAME##_func(regmatch_t MATCHES[]) { \
        ...
        if(sess->getint(sess, "authorized")!=1 || getenvptr == NULL || (strlen(getenvptr) != strlen(sess->getstr(sess, "CSRFTOKEN", false)) ) || (strncmp(sess->getstr(sess, "CSRFTOKEN", false), getenvptr, strlen(sess->getstr(sess, "CSRFTOKEN", false))) != 0 ) ||/* check password change status for first login */ (sess->getint(sess, "passwordStatus")!=0) || (sess->getint(sess, "TFAEnabled")==1 && sess->getint(sess, "TFAStatus")==0)) { \
                json_object *jresp; \
                printf("Status: 401 Unauthorized \n"); \
                printf("Set-Cookie: QSESSIONID=; path=/; secure;HttpOnly;Samesite=Lax\n"); \
                qcgires_setcontenttype(req, "application/json"); \
...
void NAME##_func(regmatch_t MATCHES[]) { \
    ...
    /*get Cookie alone from qcgireq_parse for session validation */ \
    req = qcgireq_parse_cookie(req); \
	if(req == NULL) { \
		json_object *jresp; \
		printf("Status: 400 Bad Request \r\n"); \
		qcgires_setcontenttype(req, "application/json"); \
	...
	if(sess->getint(sess, "authorized")!=1 ||  enviptr == NULL || (strlen(enviptr) != strlen(sess->getstr(sess, "CSRFTOKEN", false)) ) || (strncmp(sess->getstr(sess, "CSRFTOKEN", false), enviptr, strlen(sess->getstr(sess, "CSRFTOKEN", false))) != 0 ) ||/* check password change status for first login */ (sess->getint(sess, "passwordStatus")!=0) || (sess->getint(sess, "TFAEnabled")==1 && sess->getint(sess, "TFAStatus")==0)){ \
			json_object *jresp; \
			printf("Status: 401 Unauthorized \n"); \
            printf("Set-Cookie: QSESSIONID=; path=/; secure;HttpOnly;Samesite=Lax\n"); \
			qcgires_setcontenttype(req, "application/json"); \
    ...
    /*Get all info from qcgireq_parse for file upload to tmp directory */ \
...
#define START_HANDLER_UN_AUTHORIZED_UPLOAD(NAME, METHOD, REGEX, NUM, MATCHES, HEADERS, BASEPATH, MAXSIZEBYTES, CLEARTIME) \
map_handler mh##NAME = {.url=REGEX,.size=(unsigned long)MAXSIZEBYTES,.next=NULL}; \
void NAME##_func(); \
handler NAME##_data = {.func=NAME##_func,.method=METHOD,.regex_str=REGEX,.regex={.__buffer=0,.__allocated=0,.__used=0,.__syntax=0,.__fastmap=0,.__translate=0,.__can_be_null=0,.__regs_allocated=0,.__fastmap_accurate=0,.__no_sub=0,.__not_bol=0,.__not_eol=0,.__newline_anchor=0},.nmatch=NUM,.next=NULL};\
handler *NAME = &NAME##_data; \
...
#define END_HANDLER_UN_AUTHORIZED_UPLOAD \
end_handler: \
		qcgireq_cleanup_uploads(req); \
        END_HANDLER
