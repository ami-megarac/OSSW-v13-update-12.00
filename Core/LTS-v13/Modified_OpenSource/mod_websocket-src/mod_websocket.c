	...
	if (-1 != (gadget_cd_instance = cdrom_start((char *)nbd_dev_list[ctx->instance])))
	{
		while (1)
		{
			if (ctx->state == NBD_WAIT_REQ)
			{
				...
				{
                                        if (pthread_mutex_lock(&nbd_locks[ctx->instance]) != 0)
					{
						TCRIT("Failed to lock nbd ctx mutex\n");
					}
					...
					/*Locking is intentionally performed after the nbd_read_req() function succeeds. This read-before-lock is intentional and safe in the current design*/
                                        /* coverity[thread1_overwrites_value_in_field : FALSE] */
                                        ctx->state = NBD_WAIT_RES;
                                        pthread_mutex_unlock(&nbd_locks[ctx->instance]);
				}
				else
				{
...
void *WebSocketProxy() {
	...
	while (1) {
		...
		if (ws_ctx_cd->sockfd != -1) {
			if ((ws_ctx_cd->sockfd >= 0) && FD_ISSET(ws_ctx_cd->sockfd, &rdlist)) {
				read_again:
				...
				//Send data to server or USB driver which is received from client
				{
					...
					if (TempResPkt->CommandPkt.OpCode != MEDIA_SESSION_DISCONNECT && TempResPkt->CommandPkt.OpCode != AUTH_CMD && TempResPkt->CommandPkt.OpCode != DEVICE_INFO && TempResPkt->CommandPkt.OpCode != CDROM_KEEPALIVE_SCSI_CMD) {
						...
						else if (TempResPkt->CommandPkt.OpCode == 0xA8) //SCSI Read(12)
						{
							...
							if (pthread_mutex_lock(&nbd_locks[ws_ctx_cd->nbd_ctx.instance]) != 0) 
							{
								TCRIT("Failed to lock nbd ctx mutex before write_res\n");
							} else {
								if (nbd_write_res(&ws_ctx_cd->nbd_ctx) != -1)
								{
									ws_ctx_cd->nbd_ctx.state = NBD_WAIT_REQ;
                                                                        pthread_mutex_unlock(&nbd_locks[ws_ctx_cd->nbd_ctx.instance]);
								}
								else
								{
                                                                        pthread_mutex_unlock(&nbd_locks[ws_ctx_cd->nbd_ctx.instance]);
		...
		if ((ws_ctx_cd->targetfd >= 0) && FD_ISSET(ws_ctx_cd->targetfd, &rdlist)) {
		...
		if(ws_ctx_cd->sockfd != -1)
		{
			if (((ws_ctx_cd->sockfd >= 0) && FD_ISSET(ws_ctx_cd->sockfd, &rdlist)) || ((ws_ctx_cd->targetfd >= 0) && FD_ISSET(ws_ctx_cd->targetfd, &rdlist))) {
				NanoSleep(1);
			}
		}
	}
...
void *ReadFromWebClient(void *instance) {
	...
	while (1) {
	  if(ws_ctx->ser_stopped == 1)
		...
		if ((client >= 0) && FD_ISSET(client, &exlist)) {
		...
		// read fd is set for client, so read data from client socket
		if ((client >= 0) && FD_ISSET(client, &rdlist)) {
			read_again:
			//when full websocket frame is not received in single read
			//should read the socket directly without going for select()
			//removing "goto" jump will break huge data transfer
...
void *WritetoWebClient(void *instance) {
	...
	while (1) {
	  if(ws_ctx->cli_stopped == 1)
		...
		if ((server >= 0) && FD_ISSET(server, &exlist)) {
		...
		// read fd is set for server, so read data from server socket
		if ((server >= 0) && FD_ISSET(server, &rdlist)) {
...
int WaitOnClientSocket(int server_stopped, int socket, fd_set *rfd)
{
	...
	while(1)
	{
		...
		if ((socket >= 0) && FD_ISSET(socket, rfd)) break;
	}
	return 0;
}
/* EOF */
