...
FLAG_FILE=/conf/.openssl_copied
FLAG_FIPS_DEFAULT=/usr/local/bin/.FIPS_Default_Flag
...
if [ -f "$FIPS_FILE" ]; then
    ...
    if [ ! -f "$FLAG_FIPS_DEFAULT" ]; then
   		bash /usr/local/bin/openssl_fips_status_switch.sh off
    fi
fi
