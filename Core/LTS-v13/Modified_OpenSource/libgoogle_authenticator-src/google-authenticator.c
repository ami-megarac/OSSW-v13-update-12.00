...
//                 sizeof(SECRET)]; /* -s, --secret=<file> Specify a non-standard file location */
	...
	int sec_fn_len= sizeof(SECRET_PATH) + sizeof(channel) +strlen(username) +sizeof(SECRET);
	char *secret_fn=malloc(sec_fn_len);
    memset(secret_fn, 0, sec_fn_len);

    char *secret_ch=malloc(sizeof(SECRET_PATH) + sizeof(channel));
    memset(secret_ch,0,sizeof(SECRET_PATH) + sizeof(channel));
    ...
    */
    ...
    ret = safe_system("chmod -R 777 /conf/google_otp");
    if (ret == -1)
    {
        perror("system");
        goto errout;
...
errout:
    if (fd >= 0) {
        /* Reason: After the other "close" will not reach here. */
        /* coverity[double_close: FALSE] */
        close(fd);
    }
