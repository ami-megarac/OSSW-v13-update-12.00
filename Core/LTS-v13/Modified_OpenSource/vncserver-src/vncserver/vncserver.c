...
#define VNC_CRASH_SCREEN_DIRECTORY "/var/bsod"  // Directory where crash screen images are being stored
#define VNC_NO_SIGNAL_FILE "/etc/nosignal.jpg"  // "no signal" image file
...
static void clientgone(rfbClientPtr cl) {
	...
	if ( cl->clientData != NULL ) {
		...
		pthread_mutex_lock(&actvSessCount_mutex);
		actvSessCount--;
		pthread_mutex_unlock(&actvSessCount_mutex);
...
void adoptResolutionChange() {
	...
	if( isResolutionChanged ) {
		...
		getVideoBufferOffset(&video_buff_offset);
		rfbScreen->frameBuffer = (char*) fb_info.fbmem+video_buff_offset;
#endif
	}
	//update screen information and intimate all active clients
	prevFormat = rfbScreen->serverFormat;
	if (maxx & 3)
		printf("\n updated width [%d] is not a multiple of 4.\n", maxx);

	rfbScreen->width = maxx;
	rfbScreen->height = maxy;
	rfbScreen->bitsPerPixel = rfbScreen->depth = 8 * bpp;
	rfbScreen->paddedWidthInBytes = maxx * bpp;

	//rfbInitServerFormat(screen, bitsPerSample);

	if (memcmp(&rfbScreen->serverFormat, &prevFormat,
	...
	if (rfbScreen->cursorX >= maxx)
		rfbScreen->cursorX = maxx - 1;
	if (rfbScreen->cursorY >= maxy)
		rfbScreen->cursorY = maxy - 1;
	...
	rfbFramebufferUpdateMsg *fu = (rfbFramebufferUpdateMsg *)cl->updateBuf;
		...
		/* Reason for false positive - Unlocking cl->updateMutex before calling rfbSendNewFBSize function will cause functionality break */
                /* coverity[double_lock : FALSE] */
		if (!rfbSendNewFBSize(cl, cl->scaledScreen->width, cl->scaledScreen->height))
		{
			if (rfbScreen->displayFinishedHook)
				rfbScreen->displayFinishedHook(cl, FALSE);
			rfbReleaseClientIterator(iterator);
			return;
		}
        /* Reason for false positive - Unlocking cl->outputMutex before calling rfbSendUpdateBuf function will cause functionality break */
        /* coverity[double_lock : FALSE] */
		if(rfbSendUpdateBuf(cl) == 0)
		{
			printf("\n[%s:%d] Error: rfbSendUpdateBuf failed!", __FUNCTION__, __LINE__);
		}
#endif

		TSIGNAL(cl->updateCond);
		/* Reason for false positive - If the Unlock cl->updateMutex operation does not occur, it will cause a functionality break */
...
static void handleMouseEvent(int buttonMask, int x, int y, rfbClientPtr cl)
{
	struct sysinfo sys_info;
	int ioctl_retval = -1;
	/* Reason for false positive - The variable 'powerCons' remains unmodified and is utilized in a conditional check.*/
...
int getJpegEoiIndex(char * src_buff, int src_len,rfbClientPtr cl)
{
	int i = 0;
...
static void sendJpegFrameToClients(rfbClientPtr cl)
{
	...
	if (cl->viewOnly==TRUE && isRfbSendDataFailed == 3){
		return;
	}
	...
	if (!cl->tightEncodingSupport)
	{
		...
		if ((g_tile_info.compressed_size = getJpegEoiIndex(rfbScreen->frameBuffer, g_tile_info.compressed_size,cl)) <= 0)
		{
			rfbScreen->frameBuffer = NULL;
			return;
		}
...
void* updateVideo()
{
	...
	int count = 0;
		...
		maxRectColumn = (abs(maxx/TILE_WIDTH)) /2;
	while(1){
		if(sem_wait(&mStartCapture) == -1){
			break;
		}
		if(sysinfo(&incoming) != 0)
		{
			printf("\n sysinfo function failed , Unable to get incoming uptime \n");
		}
		...
		while(1){
			...
			if(powerCons == 1){
				...
				if(RESOLUTION_CHANGED == capture){
					...
					else if(rfbScreen->frameBuffer != (char*) fb_info.fbmem+video_buff_offset ){
						rfbScreen->frameBuffer = (char*) fb_info.fbmem+video_buff_offset;
...
int main(int argc, char** argv) {
	...
	InitSignals();
	FILE *pid_fp = fopen("/var/run/vncserver.pid", "w");
	if (pid_fp != NULL) {
		int ret = fprintf(pid_fp, "%d\n", getpid());
		if (ret < 0) {
			printf("Unable to write to pid file for vncserver\n");
		}
		fclose(pid_fp);
	}
	if( DisableAutoSwitch == 1 ) {
	...
	if(chmod(VNC_CMD_PIPE,S_IRUSR | S_IWUSR)==-1)
	{
		printf("Error in changing file %s permission\n",VNC_CMD_PIPE);
	}
	...
	if(chmod(VNC_AMI_CMD_PIPE,S_IRUSR | S_IWUSR)==-1)
	{
		printf("Error in changing file %s permission\n",VNC_AMI_CMD_PIPE);
	}
...
//	}
	...
	/* Reason for false positive - The function 'vncrfbProcessEvents' returns true only.*/
...
void saveBsodCapturedata(void* p_data_buf)
{
	...
	if( (p_data_buf != NULL )&& (g_tile_info.compressed_size !=0))
	{
		...
		if(chmod(CRASH_SCREEN_FILE_JPEG,S_IRUSR | S_IWUSR)==-1)
		{
			printf("Error in changing file %s permission\n",CRASH_SCREEN_FILE_JPEG);
		}
		...
		if (read(vncpipe, &cmd, sizeof(cmd)) > 0) {
			...
			switch (cmd.cmd)
			{
				...
				case RECV_VNC_CONF:
					if(cmd.datalen > 0)
					{
						/* Reason for false positive - The number of bytes attempt to be read will be limited to cmd.datalen - Reason*/
				...
				case DISCONNECT_VNC_SESSION:
					...
					/* Reason for false positive - The number of bytes attempt to be read will be limited to sizeof(vnc_clean_session_t) */
					/* coverity[overrun-buffer-arg : FALSE] */
					/* coverity[suspicious_sizeof : FALSE] */
					if(read(vncpipe, &vnc_session, sizeof(vnc_clean_session_t)) > 0)
					{
						/* Reason for false positive - vnc_session.reason is depending on status which is 0 or 1  it in limited range */
						/* coverity[remediation : FALSE] */
				...
				case VNC_PASSWORD_CMD:
					...
					case VNC_BSOD_CAPTURE:
						...
						if( actvSessCount == 0 )
						{
							...
							// Check if the crash screen directory exists
							if (stat(VNC_CRASH_SCREEN_DIRECTORY, &buf) != 0)
							{
								if(mkdir(VNC_CRASH_SCREEN_DIRECTORY, 0755) == -1)
								{
									printf("Unable to create %s\n",VNC_CRASH_SCREEN_DIRECTORY);
									break;
								}
							}

							if(CAPTURE_VIDEO_NO_SIGNAL == capture)
							{
								if(copyFile(VNC_NO_SIGNAL_FILE, CRASH_SCREEN_FILE_JPEG) != 0)
								{
									printf("Unable to copy JPEG file\n");
									break;
								}
							}
							...
							else if (CAPTURE_VIDEO_NO_SIGNAL != capture)
							{
...
static enum rfbNewClientAction newclient(rfbClientPtr cl) {
	...
	if ( g_kvm_client_state == VNC ) {
		...
		/* Reason for false positive - The variable actvSessCount is unmodified while accessing it here*/
		/* coverity[remove_unlocked_check : FALSE] */
...
#ifndef CONFIG_SPX_FEATURE_VIDEO_RVAS
				isNewSlaveSession = TRUE;
#endif
...
int send_session_info( int index) {
	...
	Send_Session_Info sendSessionInfo;
	if( access(VNC_AMI_CMD_PIPE, F_OK) < 0 ) {
		return -1;
	}
	...
	if(AmiVNCPipe != -1)
	{
		sendSessionInfo.comm_info.cmd = SEND_SESSION_INFO ;
		sendSessionInfo.comm_info.datalen = sizeof(VNCSessionInfo);
		sendSessionInfo.comm_info.status = index ;
		memcpy((void*)&(sendSessionInfo.sessionInfo) , (void*)&sessionInfo[index] , sizeof(VNCSessionInfo));
	        select_sleep(0, 100000);  /* Added sleep to sync between read and write function's of vncserver and vnc_ami server*/
		if(write(AmiVNCPipe,&sendSessionInfo,sizeof(Send_Session_Info)) == -1)
		{
			printf("Unable to write into %s pipe\n",VNC_AMI_CMD_PIPE);
			return -1;
		}
...
int clear_vnc_session( int sessionIndex, int reason)
{
	...
	if(AmiVNCPipe != -1)
	{
		...
		vnc_clean_session.comm_info.datalen = sizeof(vnc_session_index_t);
